...
&nbsp;
  ...
  try {
    ...
    throw new Error(
      `dev-server failed to load. Please ensure all optional dev-server dependencies (fastify, @fastify/static, @fastify/http-proxy) are installed.${error.message ? ` Original error: ${error.message}` : <span class="branch-1 cbranch-no" title="branch not covered" >""}</span>`,
    );
  }
  let livereload: any;
  try {
