    ...
    records: {
        sys_db_object: {
            ...
            async toFile(record, { descendants, config, transform }) {
                ...
                const skipDictionaryAndDbObject =
                    !config.emitDictionary && !isAugmentation && config.type !== 'configuration'
                ...
                const [
                    ...
                    skipDictionaryAndDbObject
                        ? Promise.resolve([])
                        : generateRecordXml(columns, config, transform, ['_bootstrap']),
                    skipDictionaryAndDbObject || isAugmentation
                        ? Promise.resolve([])
