...
function validateArrayElementType(
    ...
    if (!ALLOWED_ARRAY_TYPES.includes(elementTypeString)) {
        diagnostics.error(
            elementType,
            `'${elementTypeString}' type fields are not allowed inside FlowArray structures. Allowed types are: ${ALLOWED_ARRAY_TYPES.join(', ')}.`
        )
    }
}
...
function validateObjectElementType(
    ...
    if (!ALLOWED_OBJECT_TYPES.includes(elementTypeString)) {
        diagnostics.error(
            elementType,
            `'${elementTypeString}' type fields are not allowed inside FlowObject or FlowArray structures. Allowed types are: ${ALLOWED_OBJECT_TYPES.join(', ')}.`
        )
    }
}
