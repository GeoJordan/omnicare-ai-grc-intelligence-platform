...
async function activateFlows(context: PostInstallContext): Promise<void> {
    ...
    if (!response.ok && response.status !== 422) {
        ...
        if (msg.includes('does not represent any resource')) {
            logger.debug(
                'Flow activation endpoint not found. Make sure you have the latest ServiceNow IDE installed on your instance.'
            )
            return
        }
    ...
    const body = (await response.json()) as { result: ActivateFlowsResult }
    ...
    for (const r of failures.slice(0, MAX_LOGGED_FAILURES)) {
        logger.warn(
            `  Failed: ${r.flow_name ?? r.sys_id} — ${r.message ?? ''}${r.errorMessage ? `: ${r.errorMessage}` : ''}`
        )
    }
