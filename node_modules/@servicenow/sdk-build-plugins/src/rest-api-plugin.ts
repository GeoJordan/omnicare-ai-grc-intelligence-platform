...
function findReclaimedRecords(records: Record[], orphanedRecords: Record[]) {
    return records
        .filter(
            (record) =>
                record.getCreator()?.getName() === RecordPlugin.getName() &&
                !orphanedRecords.find((orphan) => orphan.getId().getValue() === record.getId().getValue())
        )
        .map((record) => new DeletedShape({ source: record.getSource() }))
}
