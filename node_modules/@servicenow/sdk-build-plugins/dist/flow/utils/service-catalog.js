...
function isCatalogAction(actionDefinition) {
    // When actionDefinition is a Record (during XML→Fluent transform), getValue() returns
    // the record's data object, not its sys_id. Use getId() to extract the sys_id in that case.
    // When it's a StringShape (raw XML import without existing Fluent project), getValue() returns the sys_id string directly.
    const sysId = actionDefinition?.ifRecord()?.getId().getValue() ?? actionDefinition?.ifString()?.getValue();
