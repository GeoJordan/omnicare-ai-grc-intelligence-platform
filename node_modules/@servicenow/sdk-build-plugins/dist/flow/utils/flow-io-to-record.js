        ...
        if (callExpr.getCallee() === 'FlowObject' || callExpr.getCallee() === 'FlowArray') {
            ...
            if (isOutput) {
                ...
                const systemAttributes = {
                    ...
                    uiTypeLabel: (0, flow_constants_1.getUiTypeLabel)(internal_type),
                ...
                additionalProperties['attributes'] = (0, complex_objects_1.formatComplexObjectAttributes)(userAttributes);
            }
...
function validateAndPopulateChoiceProperties(config, additionalProperties, diagnostics) {
    ...
    let choiceValue = flow_constants_1.ChoiceDropdown.DROPDOWN_WITH_NONE;
    ...
    if (choiceTypeNode.isString()) {
        ...
        if (choiceDropdownValue === -1) {
            ...
            diagnostics.error(choiceTypeNode, `Invalid dropdown type "${choiceTypeValue}", expected one of: ${db_1.choiceDropdown.join(', ')}`);
            return;
        }
        // dropdown_without_none has no "--None--" entry, so a default value is required
        if (choiceDropdownValue === flow_constants_1.ChoiceDropdown.DROPDOWN_WITHOUT_NONE && defaultValue.isUndefined()) {
