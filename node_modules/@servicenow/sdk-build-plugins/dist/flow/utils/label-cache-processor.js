...
exports.COMPLEX_OBJECT_TYPE = void 0;
 ...
 */
exports.COMPLEX_OBJECT_TYPE = {
    FlowObject: 'object',
    FlowArray: 'array.object',
};
/**
...
function extractTriggerOutputs(triggerInstanceShape) {
    ...
    for (const [outputName, outputValue] of outputsObj.entries()) {
        ...
        const internalType = column_helper_1.COLUMN_API_TO_TYPE[columnApiName] || exports.COMPLEX_OBJECT_TYPE[columnApiName] || columnApiName;
        if (!internalType) {
            continue;
        }
...
function extractDataPillsFromChildren(children, stepUuid, labelCacheMap) {
    for (const child of children) {
        ...
        // Strip |dataType from pills in-place (both value and displayValue, same as parent-level entries)
        if (typeof child.value === 'string') {
            child.value = child.value.replace(STRIP_DATATYPE_FROM_PILL_REGEX, '{{$1}}');
            if (typeof child.displayValue === 'string') {
                child.displayValue = child.displayValue.replace(STRIP_DATATYPE_FROM_PILL_REGEX, '{{$1}}');
            }
        }
