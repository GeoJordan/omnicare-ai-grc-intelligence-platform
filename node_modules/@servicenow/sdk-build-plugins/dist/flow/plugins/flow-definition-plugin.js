...
const flow_stage_processor_1 = require("../utils/flow-stage-processor");
...
function wrapWithWfaDataPillExpression(templateExpr, source, labelCacheMap, originalString) {
    ...
    const dataPillNames = originalString ? (0, pill_shape_helpers_1.extractDataPillNames)(originalString) : [];
    if (templateExpr.is(sdk_build_core_1.PropertyAccessShape)) {
        ...
        return (0, pill_shape_helpers_1.wrapWithDataPillCall)(templateExpr, source, dataType);
    }
    if (templateExpr.is(sdk_build_core_1.IdentifierShape)) {
        return (0, pill_shape_helpers_1.wrapWithDataPillCall)(templateExpr, source);
    }
...
function extractAllPills(stringValue, uuidToIdentifierMap, source, _diagnostics, parameterName) {
    ...
    for (const match of stringValue.matchAll(pillRegex)) {
        ...
        const uuidMatch = cleanContent.match(/^(?:step\[([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})\]|([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}))\.([a-zA-Z_][a-zA-Z0-9_.]*[a-zA-Z0-9_])$/);
        if (uuidMatch) {
    ...
    records: {
        sys_hub_flow: {
            ...
            toFile(record, { config, database, self }) {
                return (0, flow_to_xml_1.generateXML)(record, { config, database, plugin: self });
            },
            ...
            async toShape(record, { descendants, diagnostics, transform, database, logger }) {
                ...
                const stages = (0, flow_stage_processor_1.buildStageShapes)(stageRecords);
                const stageInsertionMap = (0, flow_stage_processor_1.buildStageInsertionMap)(stageRecords);
                ...
                if ((0, datapill_transformer_1.detectHoistedOutputBindings)(allInstances)) {
                    logger.warn(`Hoisting detected: action instance outputs are referenced outside their DoInParallel/TryCatch scope, falling back to Record() api. Flow = ${record.getId().getValue()}`);
                    return { success: false };
                }
                ...
                const topLevelInstances = allInstances
                    .filter((instance) => (!instance.get('parent_ui_id').isDefined() ||
                    instance.get('parent_ui_id').getValue() === '') &&
                    !(instance.getTable() === 'sys_hub_flow_logic_instance_v2' &&
                        instance.get('logic_definition').asString().getValue() === flow_logic_constants_1.TRY_CATCH_CATCH_SYS_ID))
                    .sort((a, b) => order(a) - order(b));
                // Build instance shapes (without stages) first to resolve paramName
                const rawInstanceShapes = [];
                ...
                if (firstRealShape) {
                    paramName = getArrowFunctionParamName(firstRealShape) || flow_plugin_constants_1.DEFAULT_PARAM_NAME;
                }
                ...
                for (const { shape, record: inst } of rawInstanceShapes) {
                    ...
                    if (stageKeys && stageKeys.length > 0) {
                        for (const key of stageKeys) {
                            instanceShapes.push((0, flow_stage_processor_1.createStageCallShape)(key, paramName, record));
                        }
                ...
                if (stageInsertionMap.size > 0) {
                    (0, flow_stage_processor_1.insertNestedStageShapes)(instanceShapes, stageInsertionMap, paramName, order);
                }
                ...
                const callExpression = new sdk_build_core_1.CallExpressionShape({
                    ...
                    args: [
                        record.transform(({ $ }) => ({
                            ...
                            protectionPolicy: $.from('sys_policy').def(flow_plugin_constants_1.EMPTY_STRING),
            ...
            async toRecord(callExpression, { factory, diagnostics, transform, logger }) {
                ...
                const flowDefinitionRecord = await factory.createRecord({
                    ...
                    properties: flowConfiguration.transform(({ $ }) => ({
                        ...
                        access: $.def(flow_plugin_constants_1.ACCESS_DEFAULT),
                        category: $.def(flow_plugin_constants_1.EMPTY_STRING),
                        sys_policy: $.from('protectionPolicy').def(flow_plugin_constants_1.EMPTY_STRING),
                ...
                const headerStageRecords = await (0, flow_stage_processor_1.processHeaderStages)(flowConfiguration, flowDefinitionRecord, factory);
                // ------------------------------------------------------------
                ...
                for (const [, v] of (allInstances ?? []).entries()) {
                    ...
                    if (callExpr instanceof sdk_build_core_1.CallExpressionShape && callExpr.getCallee() === flow_constants_1.STAGE_API_NAME) {
                        const stageKey = (0, flow_stage_processor_1.extractStageKey)(callExpr);
                        if (!stageKey) {
                            diagnostics.error(callExpr, 'wfa.stage() argument must be a stage reference like params.stages.<key>.');
                        }
                        else if (!headerStageRecords.has(stageKey)) {
                            diagnostics.error(callExpr, `wfa.stage() references stage '${stageKey}' which is not declared in the stages config.`);
                        }
                        else {
                    ...
                    if (instanceRecord) {
                        ...
                        if (callExpr instanceof sdk_build_core_1.CallExpressionShape) {
                            ...
                            if (callee === flow_logic_constants_1.FLOW_LOGIC.IF ||
                                callee === flow_logic_constants_1.FLOW_LOGIC.ELSEIF ||
                                callee === flow_logic_constants_1.FLOW_LOGIC.ELSE) {
                                (0, flow_stage_processor_1.resolveNestedStagesInline)(callExpr, flatRecs, order, headerStageRecords, pendingStages, subflowDefNames, diagnostics);
                            }
                            if (callee === flow_logic_constants_1.FLOW_LOGIC.TRY_CATCH) {
                                (0, flow_stage_processor_1.resolveNestedStagesForTryCatch)(callExpr, flatRecs, order, headerStageRecords, pendingStages, subflowDefNames, diagnostics);
                            }
                            if (callee === flow_constants_1.SUBFLOW_INSTANCE_API_NAME) {
                                (0, flow_stage_processor_1.collectSubflowDefName)(callExpr, subflowDefNames);
                            }
                        }
                ...
                const finalisedStages = (0, flow_stage_processor_1.finalizeStageRecords)(pendingStages, headerStageRecords);
                relatedRecords.push(...finalisedStages);
                relatedRecords.push(...(await (0, flow_stage_processor_1.createSubflowStageRecords)(relatedRecords, finalisedStages.length, flowDefinitionRecord, factory, subflowDefNames)));
                ...
                const snapshotRecord = await factory.createRecord({
                    ...
                    properties: flowConfiguration.transform(({ $ }) => ({
                        access: $.def(flow_plugin_constants_1.ACCESS_DEFAULT),
                        active: $.val(false),
                        allow_high_security_roles: $.def(flow_plugin_constants_1.BOOLEAN_FALSE_STRING),
                        annotation: $.def(flow_plugin_constants_1.EMPTY_STRING),
                        attributes: $.def(flow_plugin_constants_1.EMPTY_STRING),
                        callable_by_client_api: $.def(flow_plugin_constants_1.BOOLEAN_FALSE_STRING),
                        category: $.def(flow_plugin_constants_1.EMPTY_STRING),
                        description: $.def(flow_plugin_constants_1.EMPTY_STRING),
                        flow_priority: $.from('flowPriority').def(flow_plugin_constants_1.FLOW_PRIORITY_DEFAULT),
                        ...
                        master: $.val(true),
                        name: $,
                        natlang: $.def(flow_plugin_constants_1.EMPTY_STRING),
                        outputs: $.def(flow_plugin_constants_1.EMPTY_STRING),
                        parent_flow: $.val(flowDefinitionRecord.getId().getValue()),
                        remote_trigger_id: $.def(flow_plugin_constants_1.EMPTY_STRING),
                        run_as: $.from('runAs').def(flow_plugin_constants_1.RUN_AS_DEFAULT),
                        run_with_roles: $.val(Array.from(new Set(roles.map((r) => r.ifRecord()?.getId().getValue() ?? r.getValue()))).join(',')),
                        sc_callable: $.def(flow_plugin_constants_1.BOOLEAN_FALSE_STRING),
                        status: $.val(flow_plugin_constants_1.FLOW_STATUS_DRAFT),
                        sys_policy: $.from('protectionPolicy').def(flow_plugin_constants_1.EMPTY_STRING),
                        type: isFlow ? $.def(flow_plugin_constants_1.FLOW_TYPE_FLOW) : $.def(flow_plugin_constants_1.FLOW_TYPE_SUBFLOW),
                        version: $.def(flow_plugin_constants_1.FLOW_VERSION),
                ...
                const snapshotRelatedRecords = [];
                if (isSubflow) {
                    const snapshotInputRecords = await (0, flow_variable_processor_1.processFlowInputs)(inputsConfig, snapshotRecord, factory, diagnostics, 'sys_hub_flow_snapshot');
                    snapshotRelatedRecords.push(...snapshotInputRecords);
                    const snapshotOutputRecords = await (0, flow_variable_processor_1.processFlowOutputs)(outputsConfig, snapshotRecord, factory, diagnostics, 'sys_hub_flow_snapshot');
                    snapshotRelatedRecords.push(...snapshotOutputRecords);
                }
                if (flowConfiguration.has('flowVariables')) {
                    const snapshotVarRecords = await (0, flow_variable_processor_1.processFlowVariables)(flowConfiguration.get('flowVariables').asObject(), snapshotRecord, factory, diagnostics, 'sys_hub_flow_snapshot');
