...
exports.DataLookupPlugin = void 0;
...
exports.DataLookupPlugin = sdk_build_core_1.Plugin.create({
        ...
        [DL_DEFINITION]: {
            ...
            async toShape(record, { descendants }) {
                ...
                const setRecords = descendants
                    ...
                    .filter((r) => r.get('dl_definition').equals(record.getId()));
                const matchRules = matchRecords.map((r) => r.transform(({ $ }) => ({
                ...
                const setRules = setRecords.map((r) => r.transform(({ $ }) => ({
                    $id: $.val(now_id_plugin_1.NowIdShape.from(r)),
    ...
    shapes: [
        {
            ...
            async toRecord(callExpression, { factory, diagnostics, config }) {
                ...
                if (name && name.length > 40) {
                    diagnostics.error(arg.get('name'), `Data Lookup Definition name must be 40 characters or fewer (got ${name.length}).`);
                }
                const sourceTable = arg.get('sourceTable').ifString()?.getValue() ?? '';
                const matcherTable = arg.get('matcherTable').ifString()?.getValue() ?? '';
                const scope = config.scope;
                if (sourceTable && !sourceTable.startsWith(`${scope}_`) && !(0, sdk_build_core_1.isSNScope)(scope) && scope !== 'global') {
                    diagnostics.error(arg.get('sourceTable'), `sourceTable '${sourceTable}' must be in the same scope as the data lookup definition. Expected a table starting with '${scope}_'.`);
                }
                if (matcherTable && !matcherTable.startsWith(`${scope}_`) && !(0, sdk_build_core_1.isSNScope)(scope) && scope !== 'global') {
                    diagnostics.error(arg.get('matcherTable'), `matcherTable '${matcherTable}' must be in the same scope as the data lookup definition. Expected a table starting with '${scope}_'.`);
                }
                ...
                const matchRecords = await Promise.all(matchElements.map(async (el) => {
                ...
                const setRecords = await Promise.all(setElements.map(async (el) => {
                    const rule = el.asObject();
                    return factory.createRecord({
                        source: el,
...
//# sourceMappingURL=data-lookup-plugin.js.map