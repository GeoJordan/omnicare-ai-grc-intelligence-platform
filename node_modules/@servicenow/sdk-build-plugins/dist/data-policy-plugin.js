...
exports.DataPolicyPlugin = void 0;
 ...
 */
const mergeRestrictive = (existing, incoming) => {
...
exports.DataPolicyPlugin = sdk_build_core_1.Plugin.create({
            ...
            toShape(record, { descendants, logger }) {
                ...
                descendants.query(SYS_DATA_POLICY_RULE).forEach((rule) => {
                    ...
                    if (rulesRecord[fieldNameValue]) {
                        ...
                        logger.warn(`Data Policy '${record.getId().getValue()}': field '${fieldNameValue}' has multiple rule records on the instance. ` +
                    ...
                    const ruleConfig = {
                        $id: now_id_plugin_1.NowIdShape.from(rule),
                    };
            ...
            async toRecord(callExpression, { config, factory, diagnostics }) {
                ...
                if (tableValue?.isString() && config?.scope) {
                    ...
                    if (!tableName.startsWith(`${scopeName}_`) && !(0, sdk_build_core_1.isSNScope)(scopeName) && scopeName !== 'global') {
                        diagnostics.error(arg.get('table'), `'table' property should start with scope prefix '${scopeName}_'. Data Policies in scoped apps can only be created for tables within the same scope.`);
                    }
                }
                ...
                for (const fieldName of ruleKeys) {
                    ...
                    if (!hasMandatoryProp && !hasReadOnlyProp) {
                        mandatoryValue = sdk_build_core_1.Shape.from(ruleConfig, 'ignore');
                        readOnlyValue = sdk_build_core_1.Shape.from(ruleConfig, 'ignore');
                    }
                    // Validate mutual exclusivity: mandatory and readOnly cannot both be true
                    if (mandatoryValue?.isBoolean() &&
                        mandatoryValue.getValue() === true &&
                        readOnlyValue?.isBoolean() &&
                        readOnlyValue.getValue() === true) {
                        diagnostics.error(ruleConfig, `Rule for field '${fieldName}': mandatory and readOnly cannot both be true - these are mutually exclusive`);
                        continue;
                    }
                    ...
                    if (mandatoryValue?.isBoolean() &&
                        ...
                        if (ruleTableName !== policyTableName &&
                            !ruleTableName.startsWith(`${scopeName}_`) &&
                            !(0, sdk_build_core_1.isSNScope)(scopeName) &&
                            scopeName !== 'global') {
                            diagnostics.error(ruleConfig.get('table') || ruleConfig, `Rule for field '${fieldName}': cannot set mandatory=true for table '${ruleTableName}' which is outside the current scope '${scopeName}'. For tables in a different scope than the data policy record, you cannot make a field mandatory.`);
                            continue;
                        }
                    }
...
//# sourceMappingURL=data-policy-plugin.js.map