...
exports.AliasTemplatePlugin = void 0;
...
function convertKeys(obj, converter, definedKeys) {
 ...
 */
function serializeFluentJson(v) {
 ...
 */
function mapScriptWithModuleSupport(v) {
    return (v
        .if(server_module_plugin_1.ModuleFunctionShape)
        ?.toString((n) => `${n}({{PARAMS}})`, ['aliasId', 'connectionSysId', 'jsonDefaultData', 'jsonDynamicData']) ?? v);
}
exports.AliasTemplatePlugin = sdk_build_core_1.Plugin.create({
    ...
    records: {
        sys_alias_templates: {
            async toShape(record, { transform }) {
                ...
                const preEditScript = preEditScriptField && preEditScriptField.getValue().trim() !== DEFAULT_PRE_EDIT_SCRIPT.trim()
                    ? await now_include_plugin_1.NowIncludeShape.fromRecord(record, preEditScriptField, transform)
                    : undefined;
                const onEditScript = onEditScriptField && onEditScriptField.getValue().trim() !== DEFAULT_ON_EDIT_SCRIPT.trim()
                    ? await now_include_plugin_1.NowIncludeShape.fromRecord(record, onEditScriptField, transform)
                    : undefined;
                const postProcessScript = postProcessScriptField &&
                    postProcessScriptField.getValue().trim() !== DEFAULT_POST_PROCESS_SCRIPT.trim()
                    ? await now_include_plugin_1.NowIncludeShape.fromRecord(record, postProcessScriptField, transform)
                    : undefined;
                return {
                    success: true,
                    value: new sdk_build_core_1.CallExpressionShape({
                        ...
                        args: [
                            record.transform(({ $ }) => ({
                                ...
                                defaultDataTemplate: $.from('default_data_template').map((v) => parseDbJson(v.ifString())),
    ...
    shapes: [
        {
            ...
            async toRecord(callExpression, { factory, config, diagnostics }) {
                ...
                if (arg.get('preEditScript').isUnresolved()) {
                    diagnostics.error(arg.get('preEditScript').getOriginalNode(), `Unable to resolve the preEditScript reference, ensure the imported module is within the ${config.serverModulesDir} directory.`);
                }
                if (arg.get('onEditScript').isUnresolved()) {
                    diagnostics.error(arg.get('onEditScript').getOriginalNode(), `Unable to resolve the onEditScript reference, ensure the imported module is within the ${config.serverModulesDir} directory.`);
                }
                if (arg.get('postProcessScript').isUnresolved()) {
                    diagnostics.error(arg.get('postProcessScript').getOriginalNode(), `Unable to resolve the postProcessScript reference, ensure the imported module is within the ${config.serverModulesDir} directory.`);
                }
                return {
                    success: true,
                    value: await factory.createRecord({
                        ...
                        properties: arg.transform(({ $ }) => ({
                            ...
                            dynamic_data_schema: $.from('dynamicDataSchema').map((v) => serializeFluentJson(v.ifDefined() ? v.getValue() : undefined)),
                            default_data_template: $.from('defaultDataTemplate').map((v) => serializeFluentJson(v.ifDefined() ? v.getValue() : undefined)),
                            ...
                            test_action: $.from('testAction').map((v) => (0, utils_1.toReference)(v)),
                        })),
                    }),
                };
            },
...
//# sourceMappingURL=alias-template-plugin.js.map