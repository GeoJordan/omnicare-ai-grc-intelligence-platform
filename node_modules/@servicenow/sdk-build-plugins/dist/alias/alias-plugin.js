...
const CONNECTION_TYPE_REVERSE_MAP = Object.fromEntries(Object.entries(CONNECTION_TYPE_MAP).map(([label, platform]) => [platform, label]));
 ...
 */
function hintConnectionTypeForCredential(connectionTypeShape, aliasType, diagnostics) {
    if (aliasType === 'credential' && connectionTypeShape.isDefined()) {
        diagnostics.warn(connectionTypeShape, `'connectionType' is ignored when type is 'credential'. It only applies to 'connection' type aliases.`);
    }
}
/**
 ...
 */
function resolveConnectionType(connectionTypeShape) {
 ...
 */
function computeAliasId(name, scope) {
 ...
 */
function validateChildAliasFields(arg, diagnostics) {
    ...
    if (type?.isString() && type.getValue() === 'credential' && parent?.isDefined()) {
        diagnostics.warn(parent, `'parent' field is not visible in the UI when type is 'credential'. ` +
 ...
 */
function validateConnectionOnlyFields(arg, aliasType, diagnostics) {
    if (aliasType === 'credential') {
        const multipleConnections = arg.get('multipleConnections');
        if (multipleConnections?.isDefined()) {
            diagnostics.warn(multipleConnections, `'multipleConnections' is ignored when type is 'credential'. It only applies to 'connection' type aliases.`);
        }
        const retryPolicy = arg.get('retryPolicy');
        if (retryPolicy?.isDefined()) {
            diagnostics.warn(retryPolicy, `'retryPolicy' is ignored when type is 'credential'. It only applies to 'connection' type aliases.`);
        }
    }
}
exports.AliasPlugin = sdk_build_core_1.Plugin.create({
    name: 'AliasPlugin',
    records: {
        [SYS_ALIAS]: {
            toShape(record) {
                ...
                const defaultRetryPolicy = aliasType === 'connection' ? (DEFAULT_RETRY_POLICY[connType] ?? '') : retryPolicyValue;
    ...
    shapes: [
        {
            ...
            async toRecord(callExpression, { factory, diagnostics, config }) {
                ...
                const defaultRetryPolicy = aliasType === 'connection' ? (DEFAULT_RETRY_POLICY[resolvedConnectionType] ?? '') : '';
...
//# sourceMappingURL=alias-plugin.js.map