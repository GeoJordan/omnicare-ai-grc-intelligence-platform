...
exports.RetryPolicyPlugin = void 0;
...
exports.RetryPolicyPlugin = sdk_build_core_1.Plugin.create({
    name: 'RetryPolicyPlugin',
    records: {
        [SYS_RETRY_POLICY]: {
            async toShape(record) {
                const restrictToRaw = record.get('restrict_to')?.ifString()?.getValue();
                const restrictToArray = restrictToRaw === undefined || restrictToRaw === DEFAULT_RESTRICT_TO
                    ? undefined
                    : restrictToRaw === ''
                        ? []
                        : restrictToRaw
                    ...
                    value: new sdk_build_core_1.CallExpressionShape({
                        ...
                        args: [
                            record.transform(({ $ }) => ({
                                ...
                                maxElapsedTime: $.from('max_elapsed_time').map((v) => v.ifString()?.ifNotEmpty()?.toNumber()),
                                condition: $.def(''),
    ...
    shapes: [
        {
            ...
            async toRecord(callExpression, { factory, diagnostics }) {
                ...
                if (maxElapsedTime !== undefined && maxElapsedTime > MAX_ELAPSED_TIME_SECONDS) {
                    diagnostics.error(maxElapsedTimeShape, `maxElapsedTime must not exceed ${MAX_ELAPSED_TIME_SECONDS} seconds (24 hours). Received: ${maxElapsedTime}.`);
                }
                ...
                if (conditionValue) {
                    for (const part of conditionValue.split('^')) {
                        ...
                        if (!allowedFields.some((field) => fieldPart.startsWith(field))) {
                            ...
                            diagnostics.error(conditionShape, `Condition references field '${fieldName}' which is not in restrictTo. Allowed fields: ${allowedFields.join(', ')}.`);
                            break;
                        }
                    }
                }
...
//# sourceMappingURL=retry-policy-plugin.js.map