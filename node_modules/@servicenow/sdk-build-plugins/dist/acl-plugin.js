                    ...
                    value: new sdk_build_core_1.CallExpressionShape({
                        ...
                        args: [
                            record
                                ...
                                [merge]: $.from('type', 'name').map((type, name) => resolveAclTarget(type.pipe((t) => reverseLookup(app_1.AclTypes, t.getValue())), name.asString().getValue())),
                            }))
    ...
    shapes: [
        {
            ...
            async toRecord(callExpression, { config, diagnostics, factory }) {
                ...
                const rawArgs = callExpression.getArgument(0).asObject();
                generateDiagnosticForDuplicatedAliases(rawArgs, diagnostics);
                const arg = rawArgs.withAliasedKeys(aclAliases);
                ...
                generateDiagnosticForMutuallyExclusiveProperties(arg, diagnostics);
                ...
                if (type.equals('ux_route') || type.equals('ux_page')) {
                    ...
                    if (!table.isUndefined() || !field.isUndefined()) {
                        diagnostics.warn(table.isUndefined() ? field : table, `The 'table' and 'field' properties are deprecated for '${type.getValue()}' ACLs and will be ignored. Use the 'name' property instead.`);
                    }
                }
                if (type.equals('ux_data_broker')) {
                    const dataBroker = arg.get('dataBroker');
                    const table = arg.get('table');
                    const field = arg.get('field');
                    if (dataBroker.isUndefined() && table.isUndefined() && field.isUndefined()) {
                        diagnostics.error(dataBroker, `ACL of type 'ux_data_broker' must have either a 'dataBroker' property or 'table' and 'field' properties defined`);
                    }
                    if (dataBroker.isDefined() && (table.isDefined() || field.isDefined())) {
                        diagnostics.error(dataBroker, `ACL of type 'ux_data_broker' cannot have both 'dataBroker' and 'table'/'field' properties defined`);
                    }
                    if (dataBroker.isDefined()) {
                        if (dataBroker.isString() && !(0, sdk_build_core_1.isGUID)(dataBroker.getValue())) {
                            diagnostics.error(dataBroker, `'dataBroker' must be a valid GUID or a Record<'sys_ux_data_broker'>. Received: '${dataBroker.getValue()}'`);
                        }
                    }
                }
                ...
                const acl = await factory.createRecord({
                    ...
                    properties: arg.transform(({ $ }) => ({
                        ...
                        name: $.from('name', 'table', 'field', 'dataBroker').map((n, t, f, db) => {
                            if (db.isDefined()) {
                                return db.isString() ? db.getValue() : db.asRecord().getId().getValue();
                            }
                            return n.ifString() ?? (f.ifString() ? `${t.getValue()}.${f.getValue()}` : t);
                        }),
                        type: $.map((type) => {
                            const typeKey = type.asString().getValue();
                            return app_1.AclTypes[typeKey] ?? type;
                        }),
                        operation: $.map((op) => {
                            const opKey = op.asString().getValue();
                            return app_1.AclOperations[opKey] ?? op;
                        }),
                        advanced: $.val(advanced),
                        script: $.map((v) => v.if(server_module_plugin_1.ModuleFunctionShape)?.toString((n) => `answer = ${n}({{PARAMS}})`, ['current']) ??
                            v).toCdata(),
                        security_attribute: $.from('securityAttribute').map((v) => v.ifString()?.pipe((v) => {
                            const attrKey = v.getValue();
                            return app_1.AclAttributes[attrKey];
                        }) ?? v),
                    })),
                });
                ...
                for (const role of roles) {
                    const reference = role.isString()
                        ? (0, sdk_build_core_1.isGUID)(role.getValue())
                        ...
                        : role;
                    related.push(await factory.createRecord({
                        source: callExpression,
                        table: 'sys_security_acl_role',
                        properties: {
                            sys_security_acl: acl,
                            sys_user_role: reference,
                        },
                    }));
                }
                return {
                    success: true,
                    value: acl.with(...related),
                };
            },
        },
    ],
});
function resolveAclTarget(type, name) {
    if (type in app_1.AclNamedTypes) {
        return { name };
    }
    const split = name.indexOf('.');
    const table = split === -1 ? name : name.substring(0, split);
    const field = split === -1 ? '' : name.substring(split + 1);
    if (type in app_1.AclDataBrokerType && (0, sdk_build_core_1.isGUID)(table) && !field) {
        return { dataBroker: table };
    }
...
function generateDiagnosticForDuplicatedAliases(object, diagnostics) {
    Object.keys(aclAliases).forEach((key) => {
        const alias = aclAliases[key].find((v) => object.get(v).isDefined());
        if (alias && object.get(key).isDefined()) {
            diagnostics.error(object.get(key), `Both '${key}' and its deprecated alias '${alias}' are defined. Remove '${alias}' and keep '${key}'.`);
        }
    });
}
function generateDiagnosticForMutuallyExclusiveProperties(object, diagnostics) {
    const aclType = object.get('type').asString().getValue();
    if (!(aclType in app_1.AclTypes)) {
        const table = object.get('table');
        const field = object.get('field');
        const name = object.get('name');
        if (name.isDefined() && (table.isDefined() || field.isDefined())) {
            diagnostics.error(name, `'name' and 'table'/'field' are mutually exclusive. Use either 'name' alone or 'table' with an optional 'field', not both.`);
        }
        const appliesTo = object.get('appliesTo');
        if (appliesTo.isDefined() && name.isDefined()) {
            diagnostics.error(appliesTo, `'appliesTo' can only be used with 'table' and 'field' properties. Remove 'appliesTo' when using 'name'.`);
        }
    }
}
