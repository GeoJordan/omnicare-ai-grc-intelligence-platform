...
async function getMenuItem(item, transform) {
    const obj = {};
    addProperty(obj, '$id', now_id_plugin_1.NowIdShape.from(item));
    addProperty(obj, 'label', item.get('label'));
    addProperty(obj, 'order', (0, utils_1.getFieldAsNumber)(item, 'order', 100));
    ...
    if (spPage?.ifString()?.getValue()) {
        addProperty(obj, 'page', (0, utils_1.toReference)(spPage));
    }
    ...
    if (scCategory?.ifString()?.getValue()) {
        addProperty(obj, 'scCategory', (0, utils_1.toReference)(scCategory));
    }
    const scCatItem = item.get('sc_cat_item');
    if (scCatItem?.ifString()?.getValue()) {
        addProperty(obj, 'catItem', (0, utils_1.toReference)(scCatItem));
    }
    ...
    if (kbArticle?.ifString()?.getValue()) {
        addProperty(obj, 'kbArticle', (0, utils_1.toReference)(kbArticle));
    }
    const kbCategory = item.get('kb_category');
    if (kbCategory?.ifString()?.getValue()) {
        addProperty(obj, 'kbCategory', (0, utils_1.toReference)(kbCategory));
    }
    ...
    if (recordScript && recordScript.trim() !== '') {
        const script = await now_include_plugin_1.NowIncludeShape.fromRecord(item, item.get('record_script'), transform);
        addProperty(obj, 'script', script);
    }
    return obj;
}
async function createItemRecords(itemsShape, menuId, parentItemId, factory, diagnostics) {
    ...
    for (const item of items) {
        ...
        const itemRecord = await factory.createRecord({
            ...
            properties: {
                ...
                sp_page: obj.get('page') ? (0, utils_1.toReference)(obj.get('page')) : '',
                url: obj.get('url')?.ifString()?.getValue() ?? '',
                url_target: obj.get('urlTarget')?.ifString()?.getValue() ?? '',
                sc_category: obj.get('scCategory') ? (0, utils_1.toReference)(obj.get('scCategory')) : '',
                sc_cat_item: obj.get('catItem') ? (0, utils_1.toReference)(obj.get('catItem')) : '',
                kb_topic: obj.get('kbTopic')?.ifString()?.getValue() ?? '',
                kb_article: obj.get('kbArticle') ? (0, utils_1.toReference)(obj.get('kbArticle')) : '',
                kb_category: obj.get('kbCategory') ? (0, utils_1.toReference)(obj.get('kbCategory')) : '',
                ...
                roles: (0, utils_2.getRolesString)(obj.get('roles'), diagnostics),
            },
        });
        created.push(itemRecord);
        const childItems = obj.get('childItems');
        if (childItems?.isDefined()) {
            const childRecords = await createItemRecords(childItems, undefined, itemRecord.getId(), factory, diagnostics);
            created.push(...childRecords);
        }
    }
    return created;
}
exports.SPMenuPlugin = sdk_build_core_1.Plugin.create({
    name: 'SPMenuPlugin',
    records: {
        sp_instance_menu: {
            relationships: {
            ...
            async toShape(record, { descendants, transform }) {
                ...
                const menuItems = await Promise.all(items.map(async (item) => {
                    ...
                    const childMenuItems = await Promise.all(childItems.map(async (childItem) => {
                ...
                const asyncLoadTrigger = record.get('async_load_trigger')?.ifString()?.ifNotEmpty()?.getValue() ?? 'viewport';
                        ...
                        args: [
                            record.transform(({ $ }) => ({
                                ...
                                widgetParameters: $.from('widget_parameters')
                                    ...
                                    const parsed = (0, utils_1.noThrow)(() => JSON.parse(json));
                                ...
                                placeholderDimensions: $.from('placeholder_dimensions')
                                    ...
                                    .def(''),
                                placeholderConfigurationScript: $.from('placeholder_dimensions_script').def(default_placeholder_script),
    ...
    shapes: [
        {
            ...
            async toRecord(callExpression, { factory, diagnostics }) {
                ...
                const rolesString = (0, utils_2.getRolesString)(roles, diagnostics);
                const menuRecord = await factory.createRecord({
                    ...
                    properties: menu.transform(({ $ }) => ({
                        ...
                        sp_column: $.from('column').map(utils_1.toReference).def(''),
                        ...
                        async_load_device_type: $.from('asyncLoadDeviceType').def(defaultValues.menu.asyncLoadDeviceType),
                        ...
                        placeholder_dimensions_script: $.from('placeholderConfigurationScript').def(default_placeholder_script),
                        placeholder_template: $.from('placeholderTemplate').def(default_placeholder_template),
                    })),
                });
                const itemRecords = await createItemRecords(menu.get('items'), menuRecord.getId(), undefined, factory, diagnostics);
                return {
                    success: true,
...
//# sourceMappingURL=menu-plugin.js.map