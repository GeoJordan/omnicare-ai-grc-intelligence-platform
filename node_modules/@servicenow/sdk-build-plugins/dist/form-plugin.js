...
const ANNOTATION_TYPE_MAP = buildReverseMap(ui_1.AnnotationType);
const FORMATTER_MAP = buildReverseMap(ui_1.Formatter);
...
function resolveView(view, database) {
    ...
    return {
        sysId: viewNameStr,
        name: (0, sdk_build_core_1.isGUID)(viewNameStr) ? '' : viewNameStr,
        displayValue: (0, sdk_build_core_1.isGUID)(viewNameStr) ? '' : viewNameStr,
    };
}
    ...
    records: {
        sys_ui_section: {
            ...
            toFile(section, { config, descendants, database }) {
                ...
                const { sysId: viewSysId, name: viewName, displayValue: viewDisplayValue, } = resolveView(section.get('view'), database);
        ...
        sys_ui_form: {
            ...
            toFile(form, { config, descendants, database }) {
                ...
                const { sysId: formViewSysId, name: formViewName, displayValue: formViewDisplayValue, } = resolveView(form.get('view'), database);
                const xml = (0, xmlbuilder2_1.create)().ele('record_update');
    ...
    if (typeField === 'annotation') {
        ...
        if (annotationTypeArg) {
            if (annotationTypeArg.isRecord()) {
                ...
                if (annotationTypeStr in ui_1.AnnotationType) {
                    annotationTypeValue = ui_1.AnnotationType[annotationTypeStr];
                }
    ...
    if (typeField === 'formatter') {
        ...
        if (formatterRefArg) {
            if (formatterRefArg.isRecord()) {
                ...
                if (formatterStr in ui_1.Formatter) {
                    formatterRefValue = ui_1.Formatter[formatterStr];
                }
