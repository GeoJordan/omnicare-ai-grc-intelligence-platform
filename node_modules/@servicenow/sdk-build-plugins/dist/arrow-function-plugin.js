...
class ArrowFunctionShape extends sdk_build_core_1.Shape {
    ...
    returnValue;
    implicitReturn;
    constructor({ source, parameters, statements, returnValue, implicitReturn, }) {
        super({ source });
    ...
    getCode() {
        ...
        if (this.implicitReturn && this.returnValue) {
            if (this.returnValue instanceof sdk_build_core_1.ObjectShape) {
    ...
    nodes: [
        ...
        },
        {
            ...
            async toShape(node, { transform, diagnostics }) {
                ...
                const isExpressionBody = body && !sdk_build_core_1.ts.Node.isBlock(body);
                if (isExpressionBody) {
                ...
                const hasReturn = lastStatement && sdk_build_core_1.ts.Node.isReturnStatement(lastStatement);
    ...
    shapes: [
        {
            ...
            async commit(shape, target, { commit }) {
                const originalTargetWasArrow = sdk_build_core_1.ts.Node.isArrowFunction(target);
                ...
                if (originalTargetWasArrow &&
                    !sdk_build_core_1.ts.Node.isBlock(body) &&
                    shapeStatements.length === 0 &&
                    returnValue &&
                    shape.isImplicitReturn()) {
                    if (returnValue.is(sdk_build_core_1.ObjectShape)) {
                ...
                if (!sdk_build_core_1.ts.Node.isBlock(body)) {
                ...
                const getNonReturnStatements = () => arrowTarget.getStatements().filter((statement) => !sdk_build_core_1.ts.Node.isReturnStatement(statement));
                ...
                for (const [i, statement] of shapeStatements.entries()) {
                    ...
                    if (targetStatement) {
                        ...
                        const returnIndex = arrowTarget
                            .getStatements()
                            .findIndex((node) => sdk_build_core_1.ts.Node.isReturnStatement(node));
                        const inserted = returnIndex >= 0
                ...
                let existingReturn = arrowTarget
                    .getStatements()
                    .find((statement) => sdk_build_core_1.ts.Node.isReturnStatement(statement));
                if (!returnValue) {
                ...
                if (!existingReturn) {
                    ...
                    existingReturn = sdk_build_core_1.ts.Node.isReturnStatement(inserted)
                        ? inserted
                        : arrowTarget.getStatements().find((statement) => sdk_build_core_1.ts.Node.isReturnStatement(statement));
                }
                if (existingReturn) {
