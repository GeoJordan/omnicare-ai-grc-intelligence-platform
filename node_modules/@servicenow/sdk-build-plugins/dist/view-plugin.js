    ...
    shapes: [
        {
            ...
            inspect(record, { diagnostics, logger }) {
                ...
                if (!viewName.isString() || !/^[a-zA-Z0-9_,]+$/.test(viewName.getValue())) {
                    if (sdk_build_core_1.ts.Node.isNode(viewName.getOriginalSource())) {
                        diagnostics.error(viewName, `View name can only contain alphanumeric characters`);
                    }
                    else {
                        logger.warn(`[ViewPlugin] View name '${viewName.isString() ? viewName.getValue() : ''}' in ${record.getOriginalFilePath()} can only contain alphanumeric characters`);
                    }
                }
            },
        },
