...
const RAW_CONTENT_TAGS = ['script', 'style', 'textarea'];
/**
 ...
 */
function escapeRawContent(html) {
    const doc = (0, htmlparser2_1.parseDocument)(html, { withStartIndices: true, withEndIndices: true });
    const regions = [];
    for (const tag of RAW_CONTENT_TAGS) {
        for (const el of htmlparser2_1.DomUtils.getElementsByTagName(tag, doc, true)) {
 ...
 */
function restoreRawContent(html) {
...
const escapeSingle = (s) => escapeHtml(s).replace(/\\/g, '\\\\').replace(/'/g, "\\'");
const escapeDouble = (s) => escapeHtml(s).replace(/\\/g, '\\\\').replace(/"/g, '\\"');
const getTranslationMessages = (fs, config, rootDir, htmlRelPathFromClientDir) => {
    ...
    const translationsRelPath = htmlRelPathFromClientDir.replace(/\.html$/, isomorphic_rollup_1.TRANSLATIONS_SUFFIX);
    const translationsPath = sdk_build_core_1.path.join(rootDir, config.staticContentDir, translationsRelPath);
    try {
...
const translationsJellyScript = (scope, messages) => `
    ...
    shapes: [
        {
            ...
            async toRecord(callExpression, { config, factory, diagnostics, fs, project, logger }) {
                ...
                if (!isGlobalScope && !(0, sdk_build_core_1.isSNScope)(scope) && !endpoint.getValue().startsWith(`${scope}_`)) {
                ...
                let sourceFilePaths = [];
                let assetNames = [];
                let vendorAssetNames = [];
                let htmlRelPathFromClientDir;
                const clientAbsDir = sdk_build_core_1.path.join(project.getRootDir(), config.clientDir);
                const computeRelFromClientDir = (htmlIncludePath) => {
                    const sourceFileDir = sdk_build_core_1.path.dirname(callExpression.getOriginalNode().getSourceFile().getFilePath());
                    const absoluteHtmlPath = sdk_build_core_1.path.resolve(sourceFileDir, htmlIncludePath);
                    return sdk_build_core_1.path.relative(clientAbsDir, absoluteHtmlPath).replace(/\\/g, '/');
                };
                ...
                if (htmlShape.is(now_include_plugin_1.NowIncludeShape)) {
                    const includePath = htmlShape.as(now_include_plugin_1.NowIncludeShape).getPath();
                ...
                if (html.trimStart().startsWith(HTML_IMPORT_PREFIX)) {
                    ...
                    const moduleSpecifier = sdk_build_core_1.ts.Node.isIdentifier(htmlNode)
                        ? htmlNode
                            .getSymbol()
                            ?.getDeclarations()[0]
                            ?.getParent()
                            ?.asKind(sdk_build_core_1.ts.SyntaxKind.ImportDeclaration)
                            ?.getModuleSpecifierValue()
                        : undefined;
                    if (moduleSpecifier?.endsWith('.html')) {
                        htmlRelPathFromClientDir = computeRelFromClientDir(moduleSpecifier);
                        const sourceFileDir = sdk_build_core_1.path.dirname(htmlNode.getSourceFile().getFilePath());
                        const absoluteHtmlPath = sdk_build_core_1.path.resolve(sourceFileDir, moduleSpecifier);
                ...
                if (html) {
                    ...
                    if (htmlRelPathFromClientDir) {
                        const messages = getTranslationMessages(fs, config, project.getRootDir(), htmlRelPathFromClientDir);
                ...
                if (sourceFilePaths.length > 0) {
                    ...
                    if (vendorAssetNames.length > 0) {
                        const artifactRecord = sourceArtifactRecords.find((r) => r.getTable() === 'sn_glider_source_artifact');
                        if (artifactRecord) {
                            const vendorM2ms = await Promise.all(vendorAssetNames.map((name) => createAssetArtifactM2mRecord(artifactRecord, name, record, {
                                factory,
                                config,
                            })));
...
const getUIPageSourceFilePaths = (htmlFilePath, fs, logger, config, rootDir) => {
    ...
    try {
        ...
        const clientAbsDir = sdk_build_core_1.path.join(rootDir, config.clientDir);
        const staticContentAbsDir = sdk_build_core_1.path.join(rootDir, config.staticContentDir);
        const htmlRelPath = sdk_build_core_1.path.relative(clientAbsDir, htmlFilePath);
        const manifestPath = sdk_build_core_1.path.join(staticContentAbsDir, htmlRelPath).replace(/\.html$/, '.ui-source-manifest.json');
        ...
        const entryRelativePath = sdk_build_core_1.path.relative(config.clientDir, manifest.entry).replace(/\\/g, '/');
        const entryRelativeWithoutExt = entryRelativePath.replace(/\.[^.]+$/, '');
        const entryAssetName = sdk_build_core_1.path.join(config.scope, entryRelativeWithoutExt).replace(/\\/g, '/');
        ...
        const sourceMapFilePath = sdk_build_core_1.path.join(staticContentAbsDir, `${entryRelativeWithoutExt}.jsdbx.map`);
        try {
            fs.accessSync(sourceMapFilePath);
            const sourceMapAssetName = sdk_build_core_1.path.join(config.scope, `${entryRelativeWithoutExt}.js.map`).replace(/\\/g, '/');
        ...
        const vendorAssetNames = [];
        const vendors = manifest.vendors ?? [];
        for (const [fileName] of vendors) {
            const ext = sdk_build_core_1.path.extname(fileName);
            const nameWithoutExt = fileName.substring(0, fileName.length - ext.length);
            const vendorAssetName = sdk_build_core_1.path.join(config.scope, nameWithoutExt).replace(/\\/g, '/');
            vendorAssetNames.push(vendorAssetName);
            // Check for vendor sourcemap
            const vendorMapPath = sdk_build_core_1.path.join(staticContentAbsDir, `${nameWithoutExt}.jsdbx.map`);
            try {
                fs.accessSync(vendorMapPath);
                vendorAssetNames.push(sdk_build_core_1.path.join(config.scope, `${nameWithoutExt}.js.map`).replace(/\\/g, '/'));
            }
            catch {
...
async function serializeWithArtifact(metadataRecord, metadataProps, context) {
    ...
    context.descendants.query('sn_glider_source_artifact').forEach((artifact) => {
        ...
        const m2mQuery = currentM2mIds.length > 0
