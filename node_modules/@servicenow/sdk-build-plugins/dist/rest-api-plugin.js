...
const record_plugin_1 = require("./record-plugin");
...
function findReclaimedRecords(records, orphanedRecords) {
    return records
        .filter((record) => record.getCreator()?.getName() === record_plugin_1.RecordPlugin.getName() &&
        !orphanedRecords.find((orphan) => orphan.getId().getValue() === record.getId().getValue()))
        .map((record) => new sdk_build_core_1.DeletedShape({ source: record.getSource() }));
}
        ...
        sys_ws_definition: {
            ...
            async toShape(record, { descendants, transform }) {
                ...
                const reclaimedRecords = [
                    ...findReclaimedRecords(headers, orphanedHeaders),
                    ...findReclaimedRecords(parameters, orphanedParameters),
                ];
