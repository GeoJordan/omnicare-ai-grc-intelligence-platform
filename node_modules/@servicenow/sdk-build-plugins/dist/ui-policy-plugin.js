    ...
    records: {
        [SYS_UI_POLICY]: {
            ...
            toShape(record, { descendants }) {
                ...
                const relatedListActions = descendants.query(SYS_UI_POLICY_RL_ACTION).map((rlAction) => rlAction.transform(({ $ }) => ({
                    list: $.map((v) => {
                ...
                if (actions?.isArray()) {
                    ...
                    for (let i = 0; i < elements.length; i++) {
                        ...
                        if (!hasVisibleProp && !hasReadOnlyProp && !hasMandatoryProp && !hasClearedProp) {
                            diagnostics.hint(action, `Action at index ${i} has no effect — consider specifying at least one of: visible, readOnly, mandatory, or cleared`);
                        }
                ...
                if (relatedListActions?.isArray()) {
                    ...
                    for (let i = 0; i < rlElements.length; i++) {
                        ...
                        if (!hasVisibleProp && !listValue) {
                            diagnostics.hint(rlAction, `Related list action at index ${i} has no effect — consider specifying at least one of: list or visible`);
                        }
