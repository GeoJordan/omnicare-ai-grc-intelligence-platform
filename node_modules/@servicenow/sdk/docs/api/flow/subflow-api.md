...
| `internalName`     | `string`                            | -          | No       | Internal system name stored as `internal_name` in the record. If provided, this value is preserved on deploy. If omitted, Flow Designer auto-generates it from `name` on publish. Set this when a subflow has been renamed and the internal name has diverged from the display name |
