...
| Member | Description |
|--------|-------------|
| `flowLogic` | Control flow operators for branching, looping, and flow control (`if`, `elseIf`, `else`, `forEach`, `waitForADuration`, `setFlowVariables`, `assignSubflowOutputs`, `endFlow`, `exitLoop`, `skipIteration`, `doInParallel`, `tryCatch`, `appendToFlowVariables`). |
