...
> **Warning:** `referenceQualifier` is exclusive to `OverrideColumn` for overriding inherited fields. When defining a new column on a table, use `referenceQual` (on `ListColumn` or `ReferenceColumn`) instead. Using `referenceQualifier` on a new column definition will silently fail.
