...
- **deploymentSettings** (optional): `DeploymentSettings`
  ...
  **Note:** Once enabled, deploymentSettings should not be removed — similar to Now Assist Skill UI, which does not allow disabling deployment options once configured.
  - **nowAssistPanel** (optional): `NowAssistPanelConfigSimple | NowAssistPanelConfigAdvanced` — Now Assist Panel deployment (see [NowAssistPanelConfig](#nowassistpanelconfig) for details)
    - **enabled** (required): `true` — Enable NAP deployment
    - **roles** (optional): `(string | Role | DbRecord<'sys_user_role'>)[]` — Roles for NAP access. **Simple mode**: defaults to `'now_assist_panel_user'` if omitted. **Advanced mode**: left empty if omitted — the `conditionScript` alone governs access.
    - **conditionMode** (optional): `'simple' | 'advanced'` — Controls applicability evaluation. Default: `'simple'` (roles-only filtering).
    - **conditionScript** (optional, only when `conditionMode: 'advanced'`): `string` — Server-side JavaScript condition script. If omitted, a default template returning `false` is used. Must follow the `(function executeCondition(current) { ... })(current);` pattern.
...
### NowAssistPanelConfig

`NowAssistPanelConfigSimple | NowAssistPanelConfigAdvanced` — both require `enabled: true`.

- **Simple** (default): Roles-based filtering only. `roles` is optional — if omitted it defaults to `'now_assist_panel_user'`. `conditionScript` is **not allowed**.
- **Advanced** (`conditionMode: 'advanced'`): Custom IIFE script filtering. `roles` is optional and **not defaulted** — if omitted, roles are left empty and the script alone governs access. `conditionScript` is optional — defaults to a template that returns `false`.
...
NowAssistSkillConfig(
    {
        ...
        deploymentSettings: {
            ...
            nowAssistPanel: {
                ...
                // conditionScript is optional — if omitted, a default template script is used
                conditionScript: `(function executeCondition(/* glide record */ current) {
                    // Only show this skill for HR cases with category 'benefits'
                    return current.getValue('category') === 'benefits';
                })(current);`,
            },
            flowAction: true,
