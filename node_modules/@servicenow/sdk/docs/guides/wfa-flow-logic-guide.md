...
- **Nested Contexts** -- `endFlow` terminates the entire flow even when called from inside loops (`forEach`), conditionals (`if`/`else`), parallel blocks (`doInParallel`), or other nested logic.
