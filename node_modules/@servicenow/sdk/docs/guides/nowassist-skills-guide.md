...
### Now Assist Panel — Condition Mode

The `conditionMode` controls how skill applicability is evaluated:

| Mode | `conditionScript` | Behavior |
|------|-------------------|----------|
| `'simple'` (default) | Not needed | Roles-based filtering only. `roles` is **optional**; if omitted it defaults to `'now_assist_panel_user'` (the universal panel-access role). |
| `'advanced'` | Optional | Custom script-based filtering. `roles` is **optional and not defaulted** — if omitted, roles are left empty and the `conditionScript` alone governs access. If `conditionScript` is omitted, a default template returning `false` is used. |
```typescript fluent
// Simple mode (default) — roles-based filtering only
deploymentSettings: {
    nowAssistPanel: {
        enabled: true,
        // roles is optional — omitting it defaults to ['now_assist_panel_user'].
        // Provide additional roles to broaden access:
        roles: ['now_assist_panel_user', 'access_analyzer_ai_user'],
    },
}

// Advanced mode — roles and conditionScript are both optional.
// Omitting roles leaves them empty so the condition script governs access.
deploymentSettings: {
    nowAssistPanel: {
        enabled: true,
        conditionMode: 'advanced',
        // conditionScript is optional — defaults to a template returning false
        conditionScript: `(function executeCondition(/* glide record */ current) {
            // Only show this skill for high priority incidents
