...
After building and installing, read `src/fluent/generated/keys.ts` to extract the actual sys_id from `sys_ux_page_registry` for the workspace. The deployed workspace is then accessible at:
- Workspace: `/now/{path}/{landingPath}`
- Edit in UI Builder: `/now/builder/ui/experience/{workspace_sys_id}`
