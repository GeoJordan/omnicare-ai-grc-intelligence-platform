...
- Platform UI Pages -- see the `ui-page-guide` instead
...
10. **Verify the portal after install.** Confirm it loads without errors at `/<urlSuffix>?id=<homepagePageId>`.
11. **Portal URL.** After install, the portal is reachable at `https://<instance>.service-now.com/<urlSuffix>?id=<homepagePageId>`.
...
7. **Verify the portal** -- after install, confirm it loads without errors.
8. **Note the portal URL** for access.
