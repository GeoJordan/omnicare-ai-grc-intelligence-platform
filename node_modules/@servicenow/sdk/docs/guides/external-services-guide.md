...
- NOT for LDAP data imports -- see the `importing-data-guide` topic for that
...
- **Relationship to data import**: LDAP server config -> LDAP OU config -> Data Source (see the `importing-data-guide` topic for full chain).
