...
import type { Documentation, TableColumn } from './Column';
import type { Record } from './Record';
export type TableName = keyof Now.Internal.Tables;
export type Schema<T extends TableName> = Now.Internal.Tables[T] extends {
    schema: infer S extends object;
} ? S : TableColumn;
export type FullSchema<T extends TableName> = Now.Internal.Tables[T]['extends'] extends infer E extends string ? Schema<T> & FullSchema<E> : Schema<T>;
...
/** @interface */
export type Table<S extends TableColumn = TableColumn, E extends TableName | undefined = TableName | undefined> = Now.Internal.WithMetadata<{
    ...
    /**
     * Role used to wire the table's auto-generated access controls. Accepts a role name
     * (created on-demand via the platform's `dynamic_creation` script if it does not yet
     * exist) or a reference to an existing `sys_user_role` record. When omitted, the
     * platform falls back to the parent application's `user_role`.
     *
 ...
 * @group Table
 */
export declare function Table<const S extends TableColumn, const E extends TableName, const T extends Table<S, E>>(table: T & Table<S, E>): T;
export type SystemColumns = 'sys_created_on' | 'sys_created_by' | 'sys_updated_by' | 'sys_updated_on' | 'sys_mod_count' | 'sys_id';
export type TableSchemaDotWalk<T extends TableName> = `${string & (keyof FullSchema<T> | SystemColumns)}${'' | `.${string}`}`;
