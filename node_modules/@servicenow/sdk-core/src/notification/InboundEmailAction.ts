...
/**
 * Base properties shared by all inbound email action configurations
 */
type InboundEmailActionBase = {
    /**
...
/**
 * Type representing inbound email action configuration.
 *
 * Field restrictions by action type:
 * - 'record_action': Can use 'table' (required), 'fieldAction', 'script', and 'assignmentOperator' fields
 *   - Note: 'fieldAction' can only be used when 'table' is specified
 * - 'reply_email': Can use 'replyEmail' field
 *   - Note: 'fieldAction', 'script', and 'assignmentOperator' fields are not used for this action type
 *
 * Note: When importing from XML, the build plugin will default the 'action' field to 'record_action'
 * if it's not present in the XML record.
 */
/** @interface */
export type InboundEmailAction<T extends TableName> = Now.Internal.WithIdAndMetadata<
    | (InboundEmailActionBase & {
          /**
    ...
    | (InboundEmailActionBase & {
          /**
           * Action type that sends an automatic reply email
           */
          action: 'reply_email'
          table?: T
      })
 ...
 *
 * @param config - Inbound email action configuration object
 *
 * See {@link https://www.servicenow.com/docs/csh?topicname=c_InboundEmailActions.html&version=latest} for more details
 *
 * @example
 * ```ts
 * InboundEmailAction({
 ...
 * ```
 */
export function InboundEmailAction<T extends TableName>(config: InboundEmailAction<T>) {
    return config
}
