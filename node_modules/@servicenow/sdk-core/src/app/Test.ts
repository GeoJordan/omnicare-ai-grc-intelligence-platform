...
type ATFTestRun = { [key in keyof ATFType]: CategoryWithId<ATFType[key]> }

type TestSetup = Now.Internal.WithIdAndMetadata<{
    /** Test name */
    name: string
    /** Test description */
    description?: string
 ...
 *
 * @param configurationFunction - arrow function that receives the ATF step builder and defines
 *   test steps by calling methods on its categories, e.g.:
 *   * `atf.server.log({ log: 'message' })`
 *   * `atf.form.openForm({ table: 'incident', ... })`
 *   * `atf.rest.send({ ... })`
 */
export function Test(input: TestSetup, configurationFunction: (atf: ATFTestRun) => unknown) {
    return {
        input,
