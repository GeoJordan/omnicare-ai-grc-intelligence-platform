"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || (function () {
    var ownKeys = function(o) {
        ownKeys = Object.getOwnPropertyNames || function (o) {
            var ar = [];
            for (var k in o) if (Object.prototype.hasOwnProperty.call(o, k)) ar[ar.length] = k;
            return ar;
        };
        return ownKeys(o);
    };
    return function (mod) {
        if (mod && mod.__esModule) return mod;
        var result = {};
        if (mod != null) for (var k = ownKeys(mod), i = 0; i < k.length; i++) if (k[i] !== "default") __createBinding(result, mod, k[i]);
        __setModuleDefault(result, mod);
        return result;
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.NowConfig = exports.MODULE_RESOLUTION = void 0;
const app_1 = require("./app");
const z = __importStar(require("zod"));
const path_1 = require("./path");
const fs_1 = require("./fs");
const jsonschema_1 = require("jsonschema");
const json_1 = require("./json");
const compiler_1 = require("./compiler");
const guid_1 = require("./guid");
const taxonomy_1 = require("./taxonomy");
const now_config_dependencies_1 = require("./now-config-dependencies");
const util_1 = require("./util");
exports.MODULE_RESOLUTION = {
    V1: '1.0.0',
    V2: '2.0.0',
};
// ============================================================================
// Application Runtime Policy Schemas
// ============================================================================
/**
 * Network Policy Schema
 * Defines network access policies for the application (sys_arp_network_policy)
 */
const NetworkPolicySchema = z
    .object({
    $id: z
        .string()
        .regex(/^[0-9a-f]{32}$/, 'Must be a 32-character lowercase hexadecimal string')
        .optional(),
    active: z.boolean().default(true),
    policyType: z.enum([
        'csp_script_src',
        'csp_connect_src',
        'now_inbound_scoped',
        'now_inbound_global',
        'now_outbound',
    ]),
    status: z.enum(['requested', 'allowed', 'denied']),
    host: z
        .string()
        .max(253)
        .regex(/^[a-z][a-z0-9+.-]*:\/\/[^/\s]+$/, 'Host must include scheme (e.g., https://api.example.com) and cannot contain paths')
        .optional(),
    scheme: z.enum(['http', 'https', 'ws', 'wss']).optional(),
    path: z.array(z.string().max(1000)).max(100).optional(),
    resource: z
        .string()
        .max(4000)
        .regex(/^[^\s,\n\r]+\.[^\s,\n\r]+(\.[^\s,\n\r]+)?$/, 'Resource must be "processor.subprocessor" or "processor.subprocessor.context" without spaces or commas')
        .optional(),
    shortDescription: z.string().max(1024).optional(),
})
    .refine((data) => {
    // Validate conditional field requirements based on policy type
    const { policyType, host, scheme, path, resource } = data;
    switch (policyType) {
        case 'csp_script_src':
        case 'csp_connect_src':
            // Host required, resource must be empty
            if (!host || !host.trim()) {
                return false;
            }
            if (resource && resource.trim()) {
                return false;
            }
            break;
        case 'now_inbound_global':
            // Scheme required, host must be empty, path required, resource must be empty
            if (!scheme || !scheme.trim()) {
                return false;
            }
            if (host && host.trim()) {
                return false;
            }
            if (!path || path.length === 0) {
                return false;
            }
            if (resource && resource.trim()) {
                return false;
            }
            break;
        case 'now_outbound':
            // Host required, resource must be empty
            if (!host || !host.trim()) {
                return false;
            }
            if (resource && resource.trim()) {
                return false;
            }
            break;
        case 'now_inbound_scoped': {
            // Host must be empty, either path or resource (not both)
            if (host && host.trim()) {
                return false;
            }
            const hasResource = resource && resource.trim();
            const hasPath = path && path.length > 0;
            if (!hasResource && !hasPath) {
                return false;
            }
            if (hasResource && hasPath) {
                return false;
            }
            break;
        }
    }
    return true;
}, {
    message: 'Field requirements not met for policy type',
});
/**
 * Schema for Linter configuration options
 */
const LinterSchema = z
    .object({
    module: z.object({ enabled: z.boolean().default(true) }),
})
    .default({ module: { enabled: true } });
/**
 * Network Pillar Schema for Wildcard Policy
 */
const NetworkPillarSchema = z.object({
    active: z.boolean().default(false),
    networkWildcard: z
        .array(z.enum(['now_outbound', 'now_inbound_global', 'now_inbound_scoped', 'csp_connect_src', 'csp_script_src']))
        .optional(),
});
/**
 * Scripting Pillar Schema for Wildcard Policy
 */
const ScriptingPillarSchema = z.object({
    active: z.boolean().default(false),
    scriptingWildcard: z.array(z.enum(['sys_script_include', 'scriptable'])).optional(),
});
/**
 * ARL Pillar Schema for Wildcard Policy
 */
const ArlPillarSchema = z.object({
    active: z.boolean().default(false),
    arlWildcard: z
        .array(z.enum(['scheduled_job_limit', 'event_handler', 'api_transaction_limit', 'interactive_transaction_limit']))
        .optional(),
});
/**
 * Wildcard Policy Schema
 * Defines wildcard/exemption policies for the application (sys_arp_segment_policy)
 * Only one per application scope
 */
const WildcardPolicySchema = z.object({
    $id: z
        .string()
        .regex(/^[0-9a-f]{32}$/, 'Must be a 32-character lowercase hexadecimal string')
        .optional(),
    active: z.boolean().default(false),
    shortDescription: z.string().max(1024).optional(),
    record: z.boolean().default(false),
    network: NetworkPillarSchema.optional(),
    scripting: ScriptingPillarSchema.optional(),
    arl: ArlPillarSchema.optional(),
});
/**
 * Performance Policy Schema
 * Defines resource limits for the application (sys_app_resource_limit_template)
 * Only one per application scope
 */
const PerformancePolicySchema = z.object({
    $id: z
        .string()
        .regex(/^[0-9a-f]{32}$/, 'Must be a 32-character lowercase hexadecimal string')
        .optional(),
    name: z.string().max(100),
    scheduledJobLimit: z.number().int().min(1).max(100).optional(),
    eventHandlerLimit: z.number().int().min(1).max(100).optional(),
    apiTransactionLimit: z.number().int().min(1).max(100).optional(),
    interactiveTransactionLimit: z.number().int().min(1).max(100).optional(),
    mode: z.enum(['disabled', 'enforced', 'logOnly']).optional(),
});
// ============================================================================
const NowConfigSchema = z
    .object({
    scope: z.string(),
    scopeId: z.string(),
    name: z.string().optional(),
    serverModulesDir: z.string().default(path_1.path.join('src', 'server')),
    serverModulesIncludePatterns: z
        .array(z.string())
        .default([
        '**/*.ts',
        '**/*.tsx',
        '**/*.js',
        '**/*.jsx',
        '**/*.cts',
        '**/*.cjs',
        '**/*.mts',
        '**/*.mjs',
        '**/*.json',
    ]),
    serverModulesExcludePatterns: z.array(z.string()).default(['**/*.test.ts', '**/*.test.js', '**/*.d.ts']),
    modulePaths: z.record(z.string(), z.string()).default({}),
    metadataDir: z.string().default('metadata'),
    fluentDir: z.string().default(path_1.path.join('src', 'fluent')),
    clientDir: z.string().default(path_1.path.join('src', 'client')),
    typesDir: z.string().default(path_1.path.join('@types', 'servicenow')),
    generatedDir: z.string().default(`generated`),
    staticContentDir: z.string().default(path_1.path.join('dist', 'static')),
    staticContentPaths: z.record(z.string(), z.string()).default({
        'src/client/*.html': 'dist/static/*.html',
    }),
    appOutputDir: z.string().default(path_1.path.join('dist', 'app')),
    packOutputDir: z.string().default('target'),
    npmUpdateCheck: z.literal(false).or(z.number()).default(10),
    dependencies: now_config_dependencies_1.dependenciesSchema,
    ignoreTransformTableList: z.array(z.string()).default([]),
    excludeFilePatterns: z.array(z.string()).default([]),
    trustedModules: z.array(z.string()).default([]),
    tsconfigPath: z.string().optional(),
    scripts: z.record(z.string(), z.string()).default({}),
    linter: LinterSchema,
    tableOutputFormat: z.enum(['bootstrap', 'component']).default('bootstrap').optional(),
    emitDictionary: z.boolean().default(true),
    defaultLanguage: z
        .string()
        .regex(/^[a-z]{2,3}(-[a-zA-Z0-9]{2,8})*$/)
        .default('en'),
    tableDefaultLanguage: z // Deprecated — use defaultLanguage
        .string()
        .regex(/^[a-z]{2,3}(-[a-zA-Z0-9]{2,8})*$/)
        .optional(),
    moduleDir: z.string().optional(), // Deprecated
    compileOutputDir: z.string().optional(), // Deprecated
    sourceDir: z.string().optional(), // Deprecated
    transpiledSourceDir: z.string().optional(), // Deprecated
    // SYS_APP UI values
    active: z.boolean().optional(),
    accessControls: z
        .object({
        scopedAdministration: z.boolean().optional(),
        canEditInStudio: z.boolean().optional(),
        runtimeAccessTracking: z.string().optional(),
        restrictTableAccess: z.boolean().optional(),
        userRole: z.string().optional(),
        private: z.boolean().optional(),
        trackable: z.boolean().optional(),
        uninstallBlocked: z.boolean().optional(),
        hideOnUI: z.boolean().optional(),
    })
        .optional(),
    licensing: z
        .object({
        licensable: z.boolean().optional(),
        enforceLicense: z.string().optional(),
        licenseModel: z.string().optional(),
        licenseCategory: z.string().optional(),
        subscriptionEntitlement: z.string().optional(),
    })
        .optional(),
    jsLevel: z.string().optional(),
    menu: z.string().optional(),
    description: z.string().optional(),
    logo: z.string().optional(),
    guidedSetupGuid: z.string().optional(),
    installedAsDependency: z.boolean().optional(),
    packageResolverVersion: z.string().optional(),
    type: z.enum(['configuration', 'package']).default('package'),
    taxonomy: taxonomy_1.taxonomy,
    hostedPlugins: z.record(z.string(), z.string()).optional(),
    sysCode: z.string().optional(),
    // Application Runtime Policy (ARP) fields
    applicationRuntimePolicy: z.enum(['none', 'tracking', 'enforcing']).default('none'),
    networkPolicies: z.array(NetworkPolicySchema).optional(),
    wildcardPolicy: WildcardPolicySchema.optional(),
    performancePolicy: PerformancePolicySchema.optional(),
})
    .refine((data) => {
    if (!data.generatedDir.startsWith(data.fluentDir)) {
        data.generatedDir = path_1.path.join(data.fluentDir, data.generatedDir);
    }
    const sdkDirectories = [
        'serverModulesDir',
        'metadataDir',
        'fluentDir',
        'clientDir',
        'generatedDir',
        'staticContentDir',
        'appOutputDir',
        'sourceDir',
        'transpiledSourceDir',
        'packOutputDir',
        'moduleDir',
        'compileOutputDir',
    ];
    sdkDirectories.forEach((dir) => {
        if (data[dir]) {
            data[dir] = path_1.path.normalize(data[dir]);
        }
    });
    return data;
});
var NowConfig;
(function (NowConfig) {
    NowConfig.FILE_NAME = 'now.config.json';
    NowConfig.ERROR_ON_DEPRECATED_PROPS = {
        transpiledSourceDir: `NowConfig: 'modulePaths' replaces 'transpiledSourceDir', please remove 'transpiledSourceDir'
        and map original source files to transpiled output using 'modulePaths'
    
        Ex:
          "modulePaths": {
              "src/server/*.js": "dist/modules/*.js",
              "src/server/*.ts": "dist/modules/*.js",
          }
    
        If you are not intending to do custom module transpilation, please remove 'transpiledSourceDir'
        and ensure 'serverModulesDir' is set to the directory containing modules `,
    };
    /**
     *
     * @param config Configuration Records read from now.config.json
     * @param rootDir Root project directory used to parse tsconfig from tsconfigPath
     * @param fs FileSystem
     * @param logger
     * @returns zod parsed NowConfig
     */
    NowConfig.parse = (config, logger) => {
        checkRemovedProperties(config);
        replaceDeprecatedFields(config, logger);
        return getParsedNowConfigData(config);
    };
    NowConfig.parseFromPath = (path, fsOrCompiler, logger) => {
        const nowConfig = readNowConfigJsonFile(path, fsOrCompiler);
        if (!nowConfig) {
            throw new Error(`Failed to parse config file at path ${path}`);
        }
        return NowConfig.parse(nowConfig, logger);
    };
    NowConfig.parseFromDirectory = (dir, fs, logger) => {
        return NowConfig.parse(readNowConfigJsonFile(NowConfig.getFilePath(dir), fs) ?? (0, app_1.parsePackageJson)(dir, fs)['now'] ?? {}, logger);
    };
    NowConfig.exists = (dir, fs) => {
        if (fs_1.FileSystem.existsSync(fs, NowConfig.getFilePath(dir))) {
            return true;
        }
        else if (fs_1.FileSystem.existsSync(fs, path_1.path.join(dir, 'package.json')) && (0, app_1.parsePackageJson)(dir, fs)['now']) {
            return true;
        }
        return false;
    };
    NowConfig.getFilePath = (rootDir) => {
        return path_1.path.join(rootDir, NowConfig.FILE_NAME);
    };
    function create({ scope, scopeId, ...rest } = {}) {
        return NowConfigSchema.parse({
            scope: scope ?? 'global',
            scopeId: scopeId ?? (0, guid_1.GUID)(),
            ...rest,
        });
    }
    NowConfig.create = create;
    function load({ rootDir, compiler, fileSystem, logger, }) {
        const configFilePath = NowConfig.getFilePath(rootDir);
        if (compiler.getSourceFile(configFilePath)) {
            return NowConfig.parseFromPath(configFilePath, compiler, logger);
        }
        if (NowConfig.exists(rootDir, fileSystem)) {
            return NowConfig.parseFromPath(configFilePath, fileSystem, logger);
        }
        return NowConfig.create();
    }
    NowConfig.load = load;
    function moduleResolutionPath(config, packageJson, isDependency, ...modulePathSegments) {
        if (NowConfig.legacyPackageResolution(config)) {
            if (isDependency) {
                return path_1.path.join(config.scope, ...modulePathSegments);
            }
            return path_1.path.join(config.scope, packageJson.name, packageJson.version, ...modulePathSegments);
        }
        return path_1.path.join(config.scope === 'global' ? path_1.path.sep : '', config.scope, packageJson.name, packageJson.version, ...modulePathSegments);
    }
    NowConfig.moduleResolutionPath = moduleResolutionPath;
    function legacyPackageResolution(config) {
        if (config.packageResolverVersion) {
            if (config.scope === 'global' && config.packageResolverVersion === exports.MODULE_RESOLUTION.V1) {
                throw new Error(`Global modules must use packageResolverVersion: '2.0.0' or higher`);
            }
            return config.packageResolverVersion === exports.MODULE_RESOLUTION.V1;
        }
        if (config.scope === 'global') {
            return false;
        }
        return true;
    }
    NowConfig.legacyPackageResolution = legacyPackageResolution;
    function getRecordScope(config, filePath) {
        if (!(0, util_1.isSNScope)(config.scope) || !config.hostedPlugins) {
            return { scope: config.scope, scopeId: config.scopeId };
        }
        const matchingDir = Object.keys(config.hostedPlugins).find((pluginDirectory) => filePath.includes(path_1.path.join(config.fluentDir, 'hosted_plugins', pluginDirectory)));
        if (!matchingDir) {
            return { scope: config.scope, scopeId: config.scopeId };
        }
        return { scopeId: config.hostedPlugins[matchingDir] };
    }
    NowConfig.getRecordScope = getRecordScope;
    function getHostedDirectory(config, filePath) {
        if (!(0, util_1.isSNScope)(config.scope) || !config.hostedPlugins) {
            return;
        }
        const normalizedPath = path_1.path.normalize(filePath);
        const match = Object.keys(config.hostedPlugins).find((pluginDirectory) => normalizedPath.includes(path_1.path.join('hosted_plugins', pluginDirectory)));
        if (!match) {
            return;
        }
        return path_1.path.join('hosted_plugins', match);
    }
    NowConfig.getHostedDirectory = getHostedDirectory;
    function getConditionalDirectory(config, filePath) {
        if (!(0, util_1.isSNScope)(config.scope)) {
            return;
        }
        const match = filePath.match(/[/\\]if[/\\][^/\\]+/);
        return match ? match[0] : undefined;
    }
    NowConfig.getConditionalDirectory = getConditionalDirectory;
})(NowConfig || (exports.NowConfig = NowConfig = {}));
const NOW_CONFIG_SCHEMA = require('../now.config.schema.json');
function getNowConfigFileValidationResults(json) {
    const validator = new jsonschema_1.Validator();
    return validator.validate(json, NOW_CONFIG_SCHEMA);
}
function getParsedNowConfigData(config) {
    const parsedNowConfig = NowConfigSchema.safeParse(config);
    if (!parsedNowConfig.success) {
        let errMsg = `Invalid '${NowConfig.FILE_NAME}' file:\n`;
        parsedNowConfig.error.errors.map((err) => (errMsg += `${err.path}: ${err.message}\n`));
        throw new Error(errMsg);
    }
    return parsedNowConfig.data;
}
function validateNowConfigJsonFile(filePath, fsOrCompiler) {
    const json = (0, json_1.readJson)(filePath, '', fsOrCompiler);
    if (!isObjectWithStringKeys(json)) {
        throw new Error(`Config file '${NowConfig.FILE_NAME}' is empty or malformed`);
    }
    const result = getNowConfigFileValidationResults(json);
    if (result.errors.length > 0) {
        throw new Error(`${NowConfig.FILE_NAME} - error: ${formatValidationErrorMessage(result.errors)}`);
    }
    return json;
}
function formatValidationErrorMessage(errors) {
    return errors
        .filter((error) => !error.message.includes('does not match allOf schema'))
        .map((e) => {
        if (!e.path.length) {
            if (e.message === 'is of prohibited type [object Object]') {
                return e.argument.errorMessage;
            }
            return e.message;
        }
        return `"${e.path.join('.')}" ${e.message}`;
    })
        .join(', ');
}
function readNowConfigJsonFile(filePath, fsOrCompiler) {
    const exists = fsOrCompiler instanceof compiler_1.Compiler
        ? !!fsOrCompiler.getSourceFile(filePath)
        : fs_1.FileSystem.existsSync(fsOrCompiler, filePath);
    if (exists) {
        return validateNowConfigJsonFile(filePath, fsOrCompiler);
    }
    return null;
}
function isObjectWithStringKeys(obj) {
    if (!obj || typeof obj !== 'object' || Array.isArray(obj)) {
        return false;
    }
    for (const key in obj) {
        if (typeof key !== 'string') {
            return false;
        }
    }
    return true;
}
const DEPRECATED_FIELDS = {
    moduleDir: { replacedBy: 'serverModulesDir', transfer: true },
    compileOutputDir: { replacedBy: 'appOutputDir', transfer: true },
    entitiesDir: { replacedBy: 'fluentDir', transfer: true },
    transpiledSourceDir: { replacedBy: 'modulePaths' },
    sourceDir: {},
    tableDefaultLanguage: { replacedBy: 'defaultLanguage', transfer: true },
    tableOutputFormat: {},
};
function replaceDeprecatedFields(config, logger) {
    for (const [name, { replacedBy, transfer }] of Object.entries(DEPRECATED_FIELDS)) {
        if (name in config) {
            logger.warn(`'${name}' is deprecated${replacedBy ? `, please replace with '${replacedBy}'` : ' and can be removed'}`);
            if (transfer && replacedBy) {
                config[replacedBy] = config[name];
            }
        }
    }
}
function checkRemovedProperties(config) {
    for (const [property, message] of Object.entries(NowConfig.ERROR_ON_DEPRECATED_PROPS)) {
        if (property in config) {
            throw new Error(message);
        }
    }
}
//# sourceMappingURL=now-config.js.map